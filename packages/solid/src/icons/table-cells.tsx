...
export interface TableCellsIconHandle {
  startAnimation: () => void;
  stopAnimation: () => void;
}

interface TableCellsIconProps extends JSX.HTMLAttributes<HTMLDivElement> {
  size?: number;
  ref?: (handle: TableCellsIconHandle) => void;
}
...
const TableCellsIcon = (rawProps: TableCellsIconProps) => {
  ...
  return (
    ...
    >
      <Motion.svg
        animate={resolveValues(TABLE_VARIANTS, variant())}
        transition={resolveTransition(TABLE_VARIANTS, variant())}
                fill="none"
                height={local.size}
                stroke="currentColor"
