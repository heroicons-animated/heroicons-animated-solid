...
interface TicketIconProps extends JSX.HTMLAttributes<HTMLDivElement> {
  size?: number;
  ref?: (handle: TicketIconHandle) => void;
}
...
const TicketIcon = (rawProps: TicketIconProps) => {
  ...
  return (
    ...
    >
      <Motion.svg
        animate={resolveValues(TICKET_VARIANTS, variant())}
        fill="none"
        height={local.size}
        stroke="currentColor"
        ...
        stroke-width="1.5"
        transition={resolveTransition(TICKET_VARIANTS, variant())}
        viewBox="0 0 24 24"
        width={local.size}
